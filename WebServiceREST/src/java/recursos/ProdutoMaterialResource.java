...
public class ProdutoMaterialResource {
    ...
    public String Update(ProdutoMaterial p) {
        try {
            ...
            ProdutoMaterial prm = prodMterialDAO.findById(p.getProduto().getPro_codigo(), p.getMaterial().getMtr_codigo());
            ...
            if (totalResevado < totalUnidUtilizadas) { ///caso o total utilzado seja maior q o reservado, diminui do estoque
    ...
    @GET
    @Path("{pro_id}")///id do Produto
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public List<ProdutoMaterial> findAll(@PathParam("pro_id") Integer pro_id) {
        return prodMterialDAO.findAll(pro_id, 0);
    }

    public void DevolveMateriais(int pro_id, int neg_id) {
        List<ProdutoMaterial> lsPRM = prodMterialDAO.findAll(pro_id, 0);
        if (lsPRM != null) {
